...
function CategoryForm() {
  ...
  const {status,mutate, isLoading,error, variables} = useMutation(addCategory , {onSuccess : ()=> queryClient.invalidateQueries("get-categories")});
;
  ...
  return (
    <form onChange={changeHandler} onSubmit={submitHandler} className={styles.form}>
      ...
      <button type="submit" disabled={isLoading} >اضافه کنید</button>
    </form>
  );
}
